...
st.set_page_config(page_title='Loopbaan-Agent 3.0.1',page_icon='🧭',layout='wide')
...
D={'profile_text':'','career_move':'Ik zoek een volgende stap waarin mijn brede ervaring betekenisvol kan worden ingezet. De werkelijke opdracht en maatschappelijke opgave zijn belangrijker dan een bekende functietitel.','dna':None,'strategy':None,'results':[],'diagnostics':{},'location':'Boxtel','radius':40,'per_term':5,'max_terms':10,'max_matches':20,'minimum_score':55}
for k,v in D.items():
...
st.title('🧭 Loopbaan-Agent 3.0.1');st.caption('Multi-source zoeken vanuit Loopbaan-DNA, maatschappelijke opgaven en roltypen — met Adzuna, Jooble en je eigen bibliotheek.')
...
with t3:
    if not st.session_state.dna or not st.session_state.strategy:st.info('Maak eerst Loopbaan-DNA en opportunity-strategie.')
        ...
        if st.session_state.diagnostics:
            with st.expander('Zoekdiagnostiek',expanded=True):st.json(st.session_state.diagnostics)
...
with t4:
    ...
    up=st.file_uploader('Of upload PDF, DOCX of TXT',type=['pdf','docx','txt'],key='vu');title=st.text_input('Functietitel');org=st.text_input('Organisatie')
    if up and st.button('Sla vacaturebestand op'):
        text=read_upload(up);v={'id':'file:'+vacancy_hash({'title':title,'description':text})[:16],'title':title or up.name,'organisation':org,'location':'','description':text,'url':'','source':'Bestandsimport'};store.save_vacancy(v);st.success('Vacature opgeslagen.')
