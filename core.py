...
def normalize_exploration(data: dict) -> dict:
    ...
    role_types = []
    for item in normalized.get("roltypen") or []:
        if not isinstance(item, dict):
            continue
        role_types.append(
            {
                "naam": ensure_string(item.get("naam")),
                "type": ensure_string(item.get("type")),
                "zoektermen": ensure_list(item.get("zoektermen")),
                "waarom_passend": ensure_string(item.get("waarom_passend")),
            }
        )
    ...
    normalized["roltypen"] = role_types
...
def read_single_uploaded_file(file) -> str:
    return clean_profile_text(read_file(file))
        ...
        "roltypen": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "zoektermen": {**STRING_ARRAY, "maxItems": 4},
                    "waarom_passend": {"type": "string"},
                },
                "required": [
                    "naam",
...
Zorg voor een evenwichtige zoekruimte:
- minimaal 2 leidinggevende richtingen;
- minimaal 2 adviserende of regisserende richtingen;
- minimaal 1 brugrol die niet vanzelf uit eerdere functietitels volgt;
- maximaal 3 bijna-gelijke zoektermen per roltype;
- vermijd generieke termen als alleen "manager" of "adviseur".
""",
            max_tokens=2200,
        )
        ...
        compact_jobs = [
            {
                ...
                "omschrijving": job.get("description", "")[:3500],
            }
            for job in jobs
        ]
...
class Adzuna:
    ...
    def duplicate_key(job: dict) -> str:
        """
        Herkent dezelfde vacature ook wanneer Adzuna via verschillende
        zoektermen verschillende vacature-ID's teruggeeft.
        """
        title = normalize(job.get("title", ""))
        company = normalize(job.get("company", {}).get("display_name", ""))
        location = normalize(job.get("location", {}).get("display_name", ""))

        description = normalize(job.get("description", ""))
        description_fingerprint = hashlib.sha256(
            description[:1200].encode("utf-8")
        ).hexdigest()[:16]

        # Titel + organisatie is meestal al voldoende; locatie en tekstvingerafdruk
        # voorkomen dat gelijknamige functies op verschillende locaties samenvallen.
        return f"{title}|{company}|{location}|{description_fingerprint}"
